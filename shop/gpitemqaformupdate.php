...
if($is_mobile_shop)
    ...
    $url = "./grouppurchase.php?gp_id=$gp_id&_=".get_token()."#sit_qa";

if ($w == "")
{
    $sql = "insert g5_shop_gpitem_qa
               set gp_id = '$gp_id',
                   mb_id = '{$member['mb_id']}',
                   iq_secret = '$iq_secret',
                   iq_name  = '$iq_name',
...
else if ($w == "u")
{
    if (!$is_amdin)
    {
        $sql = " select count(*) as cnt from g5_shop_gpitem_qa where mb_id = '{$member['mb_id']}' and iq_id = '$iq_id' ";
        $row = sql_fetch($sql);
        if (!$row['cnt'])
            alert("자신의 상품문의만 수정하실 수 있습니다.");
    }

    $sql = " update g5_shop_gpitem_qa
...
else if ($w == "d")
{
    if (!$is_admin)
    {
        $sql = " select iq_answer from g5_shop_gpitem_qa where mb_id = '{$member['mb_id']}' and iq_id = '$iq_id' ";
        $row = sql_fetch($sql);
        if (!$row)
    ...
    // 에디터로 첨부된 이미지 삭제
    $sql = " select iq_question, iq_answer from g5_shop_gpitem_qa where iq_id = '$iq_id' and md5(concat(iq_id,iq_time,iq_ip)) = '{$hash}' ";
    $row = sql_fetch($sql);
    ...
    $sql = " delete from g5_shop_gpitem_qa where iq_id = '$iq_id' and md5(concat(iq_id,iq_time,iq_ip)) = '{$hash}' ";
    sql_query($sql);
