...
$gp_id       = escape_trim($_REQUEST['gp_id']);
...
if($is_mobile_shop)
    ...
    $url = "./grouppurchase.php?gp_id=$gp_id&_=".get_token()."#sit_use";
...
{
    $sql = " select is_password from g5_shop_gpitem_use where is_id = '$is_id' ";
    $row = sql_fetch($sql);
...
else if ($w == "d")
{
    if (!$is_admin)
    {
        $sql = " select count(*) as cnt from g5_shop_gpitem_use where mb_id = '{$member['mb_id']}' and is_id = '$is_id' ";
        $row = sql_fetch($sql);
        if (!$row['cnt'])
    ...
    // 에디터로 첨부된 이미지 삭제
    $sql = " select is_content from g5_shop_gpitem_use where is_id = '$is_id' and md5(concat(is_id,is_time,is_ip)) = '{$hash}' ";
    $row = sql_fetch($sql);
    ...
    $sql = " delete from g5_shop_gpitem_use where is_id = '$is_id' and md5(concat(is_id,is_time,is_ip)) = '{$hash}' ";
    sql_query($sql);

    $alert_msg = "사용후기를 삭제 하였습니다.";
}

update_use_avg($gp_id);
