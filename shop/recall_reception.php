...
if ($is_member)
{
    $sql_common = " from {$g5['g5_shop_order_table']} where od_id in ( select distinct od_id from g5_shop_cart where ct_gubun = 'P' and ct_type!='' ) and mb_id = '{$member['mb_id']}' ";
}
else if ($od_id && $od_pwd) // 비회원인 경우 주문서번호와 비밀번호가 넘어왔다면
{
    $sql_common = " from {$g5['g5_shop_order_table']} where od_id in ( select distinct od_id from g5_shop_cart where ct_gubun = 'P' and ct_type!='' ) and od_id = '$od_id' and od_pwd = '$od_pwd' ";
}
...
<div id="sod_v">
    ...
    include "./orderinquiry_gp.sub.php";
    ?>
