...
<div style="background:#fff;">
	...
	<div id="general_box">
		<table border="0" cellspacing="0" cellpadding="0" width="100%" height="80px">
			...
			<tr>
				<td class="des"><?=od_status_cnt_gp("입금대기")?></td>
				<td class="des"><?=number_format(od_status_cnt_gp("결제완료"))?></td>
				<td></td>
				<td class="des"><?=number_format(od_status_cnt_gp("상품준비중"))?></td>
				<td class="des"><?=number_format(od_status_cnt_gp("배송중"))?></td>
				<td class="des"><?=number_format(od_status_cnt_gp("배송완료"))?></td>
			</tr>
		</table>
	</div>
	...
	<div class="tbl_head01 tbl_wrap" style="margin:10px 0 0 0;border-top:2px #545454 solid;font-size:11px;">
		...
		if($my_sch_sel == "입금대기"){
			...
			$search_que = " and od_status='".$my_sch_sel."' ";
		}else if($my_sch_sel == "해외배송대기"){
			$search_que = " and od_status='".$my_sch_sel."' ";
		}else if($my_sch_sel == "해외배송중"){
			$search_que = " and od_status='".$my_sch_sel."' ";
		}else if($my_sch_sel == "상품준비중"){
			$search_que = " and od_status='".$my_sch_sel."' ";
		}else if($my_sch_sel == "배송대기"){
			$search_que = " and od_status='".$my_sch_sel."' ";
		}else if($my_sch_sel == "배송중"){
			$search_que = " and od_status='".$my_sch_sel."' ";
		}else if($my_sch_sel == "배송완료"){
			$search_que = " and od_status='".$my_sch_sel."' ";
		}else if($my_sch_sel == "미입고"){
			$search_que = " and od_status='".$my_sch_sel."' ";
		}
		...
		$sql = " select * from {$g5['g5_shop_order_table']}
				 where od_id in ( select distinct od_id from g5_shop_cart where ct_gubun = 'P' )
		...
		for ($i=0; $row=sql_fetch_array($result); $i++)
		{
			...
			$cart_row = sql_fetch("select sum(ct_qty) as ct_total_qty, sum(ct_gp_soldout) as ct_total_gp_soldout, sum(ct_wearing_cnt) as ct_total_gp_wearing, it_id, ct_type, it_name, total_amount_code from {$g5['g5_shop_cart_table']} where od_id='".$row[od_id]."' order by ct_time desc limit 0, 1 ");
			//$cart_num = mysql_num_rows($cart_res);
			//$cart_num = $cart_num - 1;

			$n_stock =  $cart_row['ct_total_qty'] - $cart_row['ct_total_gp_soldout'];
			if($n_stock<=0){
				$od_status = '<font color="red">품절</font>';
				$row['od_misu'] = $row['od_send_cost'] = $row['od_send_cost2'] = 0;
			}
			...
			if($k == 0){
				$rowspan = sql_fetch("select count(*) as cnt from {$g5['g5_shop_order_table']} where combine_deli_code != '' and od_cart_price!=0 and combine_deli_code='".$row[combine_deli_code]."' and combine_deli_status='y' and od_id in (select distinct od_id from g5_shop_cart where ct_gubun = 'P') ");
				$k = $rowspan[cnt];
			}

			if($od_status!="신청대기" && $od_status!="입금대기"){
				$row['od_receipt_price'] = $row['od_cart_price'] + $row['od_send_cost'] + $row['od_send_cost2'];
				$row['od_misu'] = 0;
			}
		?>
		...
		<tr>
			...
			<td class="my_td" style="width:50px;text-align:center;" rowspan="<?=$rowspan[cnt]?>">
				<?=item_deli_type($row[od_id], "gp", $row[combine_deli_code], "T")?></br>
			</td>
			<?}?>-->

			<td style="width:125px;padding-left:20px">
				<?=$cart_row[total_amount_code]?></br>
				...
				<img src="<?=G5_URL?>/img/my_view_bn.gif" class="layer_trigger_my" T_od_id="<?=$row[od_id]?>" status="N" dealer_status="gp">
			</td>
			<td>
				<table border="0" cellspacing="0" cellpadding="0" width="100%">
					<tr>
			...
			<td class="my_td" style="text-align:center;">
				...
				(<?php echo $cart_row['ct_total_qty']; ?>개)
				<?php if($cart_row['ct_total_gp_soldout']>0)echo '<br><font color="red">('.$cart_row['ct_total_gp_soldout'].'개 품절)</font>';?>
				<?php if($cart_row['ct_total_gp_wearing']>0)echo '<br><font color="blue">('.$cart_row['ct_total_gp_wearing'].'개 미입고)</font>';?>
			...
			<td style="background:#faf9f9;width:70px;text-align:center;font-size:9px;color:#3db2ff;" rowspan="<?=$rowspan[cnt]?>">
				...
				<?if($row[od_status] == "배송중" && $row[od_invoice]){?>
				<?php echo get_delivery_inquiry($row['od_delivery_company'], $row['od_invoice'], 'my_deli_info_bn1'); ?>
				<?}?>
			</td>
