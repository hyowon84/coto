...
<div class="list_box">
	...
	<div id="sod_frm">
		...
		<div class="tbl_head01 tbl_wrap product1">
			...
			$sql = " select a.ct_id,
					  ...
					  where a.od_id = '$s_cart_id' and (b.auc_status='1' or b.auc_status is NULL)
						and a.ct_select = '1' ";
			...
			<tr>
				<td>
					<input type="checkbox" name="ct_chk[<?php echo $k; ?>]" value="1" id="ct_chk_<?php echo $k; ?>" status="1" checked="checked">
				</td>
	...
	<section id="sod_frm_orderer">
		...
		<div class="tbl_frm01 tbl_wrap">
			...
			<tr>
				...
				<td>
					<input type="hidden" name="od_email" value="<?php echo $member['mb_email']; ?>" id="od_email" required class="frm_input required">
		...
		<div class="tbl_frm01 tbl_wrap">
			...
			<tr>
				...
				<td id="sod_frm_addr">
					...
					<input type="hidden" name="od_b_addr2" id="od_b_addr2">
					<input type="text" name="od_b_addr3" id="od_b_addr3" readonly="readonly" class="frm_input frm_address" size="40">
					...
					<td style="padding:0;border:0;width:65%;">
						...
						<tr>
							...
							<td >
								...
								// 신용카드 사용
								$is_cash_payment = false;	//2014-06-05오후 4:11 우선 무통장만 가능하게
								...
								<div style="margin:10px 0 0 0;">
									<textarea style="width:100%;height:50px;">1</textarea>
								</div>
							</td>
						</tr>
						...
						<tr class="mu_dis">
							<th>예금주</th>
							<td>
								<input type="text" name="od_deposit_name" value="<?=$member[mb_name]?>">
							</td>
						</tr>
						<tr class="mu_dis">
							<th>입금기한</th>
						...
						<tr class="mu_dis">
							...
							<td>
								<p>
								<input type="radio" name="od_tax" value="1" disabled>소득공제용&nbsp;&nbsp;
								<input type="radio" name="od_tax" value="2" disabled>지출증빙용&nbsp;&nbsp;
								...
								<input type="text" name="od_tax_hp[]" size="4" maxlength="4"> -
								<input type="text" name="od_tax_hp[]" size="4" maxlength="4"> -
								<input type="text" name="od_tax_hp[]" size="4" maxlength="4">
								</p>
							</td>
					...
					<td style="padding:17px;border:0;background:#eeeff3;" valign="top">
						<table border="0" cellspacing="0" cellpadding="0" width="100%" class="buy_all_price_tb">
							...
							<tr style="font-size:15px;">
								<td>총 결제 금액</td>
								<td align="right" style="font-size:17px;color:#fc3f00;"><span>
	...
	<div id="display_pay_button" class="btn_confirm" style="display:none">
		<input type="image" src="<?G5_URL?>/shop/img/cart_buy_all_bn.gif">
...
$(function() {
        ...
        if($(this).is(":checked")){
			...
			if(status == 1){
				$(".product1").find(".sell_price").each(function(i){
					price1_1 = $(".product1").find(".sell_price").eq(i).html().replace(",", "");
				...
				price3 = $(".price_info1").find(".price").eq(2).html().replace(",", "");
				...
				$(".product2").find(".sell_price").each(function(i){
					price1_1 = $(".product2").find(".sell_price").eq(i).html().replace(",", "");
					price1 = parseInt(price1_1) + parseInt(price1);
				});

				$(".price_info2").find(".price").eq(0).html(commaNum(price1));
				$(".all_price_info").find(".price_all").eq(0).html(commaNum(price1));
				price3 = $(".price_info2").find(".price").eq(2).html().replace(",", "");
				price3 = parseInt(price3) + price1;
	...
	$("input:checkbox[name^='ct_chk']").click(function(){
		var sell_price = $(this).parent().parent().find(".sell_price").html().replace(",", "");
		var price = 0;
		var cost = 0;
		var status = $(this).attr("status");
		var num = 0;
		...
		if(status == 1){
			price = $(".price_info1").find(".price").eq(0).html().replace(",", "");
			cost = $(".price_info1").find(".price").eq(2).html().replace(",", "");
		}else{
			price = $(".price_info2").find(".price").eq(0).html().replace(",", "");
			cost = $(".price_info2").find(".price").eq(2).html().replace(",", "");
		}
	...
	$(".price_info2").find(".price").eq(0).html(price1);
...
function cart_cnt(th, idx, old_cnt, cnt, pro_price, price1_1, price1, cart_id, status){
	...
	$.ajax({
		...
		success : function(data){
			...
			cost1 = $(".price_info1").find(".price").eq(2).html().replace(",", "");
			cost2 = $(".price_info2").find(".price").eq(2).html().replace(",", "");
			cost = parseInt(cost1) + parseInt(cost2);
			
			$(".all_price_info").find(".price_all_cost").html(commaNum(cost));

			all_price = parseInt($(".price_info1").find(".price").eq(3).html().replace(",", "")) + parseInt($(".price_info2").find(".price").eq(3).html().replace(",", ""));
...
function forderform_check(f)
{
    ...
    f.rcvr_tel2.value = f.od_b_tel.value;
    ...
    f.rcvr_add2.value = f.od_b_addr3.value;
