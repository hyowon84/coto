...
function option_selected_nobr($value, $selected, $text='')
{
	if (!$text) $text = $value;
	if ($value == $selected)
		return "<option value=\"$value\" selected=\"selected\">$text</option>";
...
function getShortExplodeValue($val,$firstExp,$locNum=0)
{
	$tmp = explode($firstExp,$val);

	return $tmp[$locNum];
}

//환율정보 호출
/*function get_currency1($from, $to){
	$xml_string = file_get_contents('http://www.webservicex.net/CurrencyConvertor.asmx/ConversionRate?FromCurrency='.$from.'&ToCurrency='.$to);
...
function get_rates_ret($num){
	$rates = get_currency1($num);

	$result = array();

	$dom = new DOMDocument;
	$dom->loadHTML("<table><tr>".$rates."</tr></table>");
...
//포프폴리오 그래프 퍼센트 리턴
function port_percent($loginid, $metal){

	$num_all = sql_fetch("select SUM(wr_4 * wr_3) as wr_4 from g5_write_portfolio where mb_id='".$loginid."' ");

	if($num_all[wr_4]){
		if($metal){
			$mt_num = sql_fetch("select SUM(wr_4 * wr_3) as wr_4 from g5_write_portfolio where mb_id='".$loginid."' and wr_2='".$metal."' ");
		}else{
			$mt_num = sql_fetch("select SUM(wr_4 * wr_3) as wr_4 from g5_write_portfolio where mb_id='".$loginid."' and wr_2 not in('GL', 'SL', 'PT', 'PD') ");
		}
...
//포트폴리오 환율 증감
function gp_rates_inc($rate, $gp_rates){
	$rates_arr = array();

	$rates = $gp_rates - $rate;
	$gp_rates_arr = explode(".", round($gp_rates, 2));
	if($gp_rates_arr[1] < 10){
		$gp_rates = $gp_rates_arr[0].".".$gp_rates_arr[1]."0";
	}else{
		$gp_rates = round($gp_rates, 2);
	}

	if($rates < 0){
		$rates_arr[0] = "<font color='blue'>".$gp_rates."</font>";
		$rates_arr[1] = "<font color='blue'>".round($rates, 2)."</font>";
	}else{
		$rates_arr[0] = "<font color='red'>".$gp_rates."</font>";
		$rates_arr[1] = "<font color='red'>".round($rates, 2)."</font>";
	}
...
// 해당 자리수 올림
function CeilGe($number,$ge=2){
	return ceil($number / pow(10,$ge)) * pow(10,$ge);
}


function getExplodeValue($val,$firstExp,$twoExp,$locNum=0)
{
	$tmp = explode($firstExp,$val);
	$tmp2 = explode($twoExp,$tmp[1]);
	return $tmp2[$locNum];
}



//APMEX, MCM 업체에 개별오더가 가능한 공구코드 확인
function getDirectBuyCode() {
	$sel_sql = "	SELECT	*
								FROM		gp_info
								WHERE		stats		IN ('00')	/*개별구매 접수 받는중일때만*/
								AND			(	choice_dealer LIKE '%$_GET[ca_id]%'	OR	choice_dealer LIKE '%ALL%' )
								ORDER BY	reg_date DESC
								LIMIT 1
	";
	$row = mysql_fetch_array(sql_query($sel_sql));
	return $row;
}
