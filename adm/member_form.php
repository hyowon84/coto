...
else if ($w == 'u')
{
	...
	$mb['mb_img'] = $mb['mb_img'];
	$mb['mb_id'] = explode("@", get_text($mb['mb_id']));
	...
	$mb_birth1 = substr($mb['mb_birth'], 0, 4);
	$mb_birth2 = substr($mb['mb_birth'], 4, 2);
	$mb_birth3 = substr($mb['mb_birth'], 6, 2);
...
<div class="tbl_frm01 tbl_wrap">
	...
	<tbody>
	<tr>
		<th scope="row"><label for="mb_img">프로필사진</label></th>
		<td><?=$mb['mb_img']?></td>
	<tr>
	...
	<tr>
		...
		<td><input type="text" name="mb_name" value="<?=$mb['mb_name']?>" id="mb_name" required class="required frm_input minlength=2" size="15" maxlength="20"></td>
	 ...
	 <tr>
		...
		<td>
			<select name="mb_birth1" class="frm_input required nospace">년
				<?
				for($i = date("Y"); $i >= date("Y")-100; $i--){
				?>
				<option value="<?=$i?>" <?if($mb_birth1 == $i){echo "selected";}?>><?=$i?></option>
				<?
				}
				?>
			</select>
			<select name="mb_birth2" class="frm_input required nospace">월
				<?
				for($i = 1; $i <= 12; $i++){
				?>
				<option value="<?=$i?>" <?if($mb_birth2 == $i){echo "selected";}?>><?=$i?></option>
				<?
				}
				?>
			</select>
			<select name="mb_birth3" class="frm_input required nospace">일
				<?
				for($i = 1; $i <= 31; $i++){
				?>
				<option value="<?=$i?>" <?if($mb_birth3 == $i){echo "selected";}?>><?=$i?></option>
				<?
				}
				?>
			</select>
		...
		<td>
			<input type="radio" name="mb_sex" value="m" <?if($mb['mb_sex'] == "" || $mb['mb_sex'] == "m"){echo "checked";}?>>남
			<input type="radio" name="mb_sex" value="w" <?if($mb['mb_sex'] == "w"){echo "checked";}?>>여
		</td>
	</tr>
	<tr>
		...
		<td>
			...
			$mbtype_sql = "	SELECT  *
											...
											ORDER BY `order` ASC
			";
			$mbtype_result = sql_query($mbtype_sql);

			while($mbtype = mysql_fetch_array($mbtype_result)) {
				$선택 = ($mbtype[code] == $mb[mb_type]) ? 'selected' : '';
				echo "<option value='$mbtype[code]' $선택>$mbtype[value]</option>";
			}

			?>
			</select>
			&nbsp;&nbsp;
			LV: <?=get_member_level_select('mb_level', 1, $member['mb_level'], $mb['mb_level'])?>
		</td>
		<th scope="row">포인트</th>
	...
	<tr>
		...
		<td>
			<input type="radio" name="mb_certify" value="1" id="mb_certify_yes" <?=$mb_certify_yes;?>>
			<label for="mb_certify_yes">예</label>
			<input type="radio" name="mb_certify" value="" id="mb_certify_no" <?=$mb_certify_no;?>>
			<label for="mb_certify_no">아니오</label>
		</td>
		<th scope="row"><label for="mb_adult">성인인증</label></th>
		<td>
			<input type="radio" name="mb_adult" value="1" id="mb_adult_yes" <?=$mb_adult_yes;?>>
			<label for="mb_adult_yes">예</label>
			<input type="radio" name="mb_adult" value="0" id="mb_adult_no" <?=$mb_adult_no;?>>
	...
	<tr>
		...
		<td colspan="3" style="line-height:2em">
			...
			<span id="mb_addr_jibeon">지번주소 : <?=$mb['mb_addr_jibeon'];?></span>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
			<input type="radio" name="mb_mailling" value="1" id="mb_mailling_yes" <?=$mb_mailling_yes;?>>
			<label for="mb_mailling_yes">예</label>
			<input type="radio" name="mb_mailling" value="0" id="mb_mailling_no" <?=$mb_mailling_no;?>>
			<label for="mb_mailling_no">아니오</label>
		</td>
		<th scope="row"><label for="mb_sms_yes">SMS 수신</label></th>
		<td>
			<input type="radio" name="mb_sms" value="1" id="mb_sms_yes" <?=$mb_sms_yes;?>>
			<label for="mb_sms_yes">예</label>
			<input type="radio" name="mb_sms" value="0" id="mb_sms_no" <?=$mb_sms_no;?>>
	...
	<tr>
		...
		<td colspan="3">
			<input type="radio" name="mb_open" value="1" id="mb_open_yes" <?=$mb_open_yes;?>>
			<label for="mb_open">예</label>
			<input type="radio" name="mb_open" value="0" id="mb_open_no" <?=$mb_open_no;?>>
