...
//							collapsible: false,
//							floatable: false,
							items:[grid_window_portfolio]		/*발주입력품목그리드*/
						}
				]	//items item end
				,
				buttons: [{
...
function updateOrderStats(stats) {
	var jsonData = "[";
	var cnt = grid_window_portfolio.getStore().data.items.length;
	var form = Ext.getCmp('frmInvoice');
	
	for(var i = 0; i < cnt; i++) {
		jsonData += Ext.encode(grid_window_portfolio.getStore().data.items[i].data)+",";
	}
...
//								form.getForm().reset();
			form.reset();
			grid_window_portfolio.getStore().removeAll();
			grid_pfItemList.getStore().load();
			winInvoice.hide();
