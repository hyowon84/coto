...
else if ($w == "u")
{
	...
	if($it['it_type1'] == "0" && $it['it_type2'] == "0" && $it['it_type3'] == "0" && $it['it_type4'] == "0" && $it['it_type5'] == "0"){
		$it_type0 = 1;
	}
}
else
...
$item_type_icon_res = sql_query("select * from {$g5['g5_item_type_icon_table']} order by no desc ");
?>
...
<section id="anc_sitfrm_ini">
	...
	<div class="tbl_frm01 tbl_wrap">
		...
		<tr>
			...
			<td colspan="2">
				일반상품 <input type="radio" name="item_status" value="1" <?//if($it['auc_status'] == "1" || $it['auc_status'] == ""){?>checked<?//}?>>
				경매상품 <input type="radio" name="item_status" value="2" <?//if($it['auc_status'] == "2"){?>checked<?//}?>>
			</td>
		</tr>
		-->
		...
		<!--
		<tr class="ac">
			<th scope="row"><label for="it_bid">입찰가</label></th>
			<td colspan="2">
				<?php// echo help("입찰가를 입력해주세요."); ?>
				<input type="text" name="it_bid" value="<?php// echo $it['it_bid']; ?>" id="it_bid" required class="frm_input required" size="10">
			</td>
		</tr>
		-->

		<tr class="ac">
			<th scope="row"><label for="it_last_date">수량</label></th>
			<td colspan="2">
				<?php echo help("수량 선택해주세요."); ?>
				<input type="text" name="it_cnt" value="<?php echo $it['it_cnt']; ?>" id="it_cnt" class="frm_input" size="7">
			</td>
		</tr>
		...
		$(function(){
			...
			<?php if($it['it_price_type']=='Y'){?>
				$(".realaddP").show();
			<?php }elseif($it['it_price_type']=='U'){?>
				$(".realUrl").show();
				$(".realaddP").show();
			<?php }elseif($it['it_price_type']=='A'){ ?>
				$(".ac").show();
			<?php }?>
		});
		</script>
		...
		<tr>
			<th scope="row"><label for="it_metal_don">금속 정보</label></th>
			<td colspan="2">
			<select name="it_metal_type" id="it_metal_type">
			<?php foreach($goodsMetalListArray as $key=>$vars){?>
			<option value="<?php echo $key?>"<?php if($key==$it[it_metal_type])echo " selected";?>><?php echo $vars?></option>
			<?php }?>
			</select> /  <input type="text" name="it_metal_don" value="<?php echo $it['it_metal_don']; ?>" id="it_metal_don" required class="required frm_input" size="5"> oz,t 
			<span id="it_metal_etc_price_view"<?if($it[it_metal_type]!="EC")echo ' style="display:none;"';?>> / 기타 금속 가격 <input type="text" name="it_metal_etc_price" value="<?php echo $it['it_metal_etc_price']; ?>" id="it_metal_etc_price" class="frm_input" size="8"> USD</span>
			</td>
		</tr>

		<tr>
			<th scope="row"><label for="it_year">간단설명</label></th>
			<td colspan="2">
				<input type="text" name="it_year" value="<?php echo get_text(cut_str($it['it_year'], 250, "")); ?>" id="it_year" required class="frm_input required" size="95">
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				...
				<input type="radio" name="it_type" value="" id="it_type" <?php echo ($it['it_type'] == "" ? "checked" : ""); ?>>
				...
				<input type="radio" name="it_type" value="<?=$item_type_icon_row[no]?>" <?php echo ($it['it_type'] == $item_type_icon_row[no] ? "checked" : ""); ?> id="it_type<?=$item_type_icon_row[no]?>">
				<label for="it_type<?=$item_type_icon_row[no]?>"><?=$item_type_icon_row[tp_name]?> <img src="<?php echo G5_URL; ?>/data/item_type_icon/<?=$item_type_icon_row[tp_img]?>" alt=""></label>
		...
		$itemEtcList = array("1"=>"발행년도","금속타입","금속함량","액면가","지름,두께","케이스포함 중량(g)","상태");
			
		for ($i=1; $i<=count($itemEtcList); $i++) { ?>
		<input type="hidden" name="it_<?php echo $i ?>_subj" value="<?php echo $itemEtcList[$i] ?>">
		<tr>
			<th scope="row"><label for="it_<?php echo $i ?>"><?php echo $itemEtcList[$i] ?></label></th>
			<td>  
				<?php echo help("입력하지 않으면 상품상세페이지에 출력하지 않습니다."); ?>
				<input type="text" name="it_<?php echo $i ?>" value="<?php echo get_text($it['it_'.$i]) ?>" id="it_<?php echo $i ?>" class="frm_input" size="40">
			</td>
			<td class="td_grpset">
	...
	<div class="tbl_frm01 tbl_wrap">
		...
		<tr>
			...
			<td>
				...
				<input type="radio" name="it_price_type" id="it_fix_price_type" value="N" <?php if($it[it_price_type]=="N")echo "checked";?>> 고정형
				<input type="radio" name="it_price_type" id="it_real_price_type" value="Y" <?php if($it[it_price_type]=="Y")echo "checked";?>> 실시간형
				<input type="radio" name="it_price_type" id="it_url_price_type" value="U" <?php if($it[it_price_type]=="U")echo "checked";?>> 실시간형 URL 
				<input type="radio" name="it_price_type" id="it_auc_price_type" value="A" <?php if($it[it_price_type]=="A")echo "checked";?>> 경매형

				 <!--<input type="radio" name="it_price_type" value="2" <?//if($it['auc_status'] == "2"){?>checked<?//}?>>-->
			</td>
			<td class="td_grpset">
		...
		<tr class="realaddP">
			<th scope="row"><label for="it_real_add_price">실시간형 추가금액</label></th>
			<td colspan="2"><input type="text" name="it_real_add_price" value="<?php echo $it['it_real_add_price']; ?>" id="it_real_add_price" class="frm_input" size="8"> 
				<select name="it_real_add_unit" id="it_real_add_unit" class="frm_input">
					<option value="W" <?if($it[it_real_add_unit] == "W"){echo "selected";}?>>원</option>
					<option value="P" <?if($it[it_real_add_unit] == "P"){echo "selected";}?>>%</option>
					<option value="D" <?if($it[it_real_add_unit] == "D"){echo "selected";}?>>$</option>
				</select>
			</td>
		</tr>

		<tr class="realUrl">
			<th scope="row"><label for="it_real_url">실시간형 URL</label></th>
			<td colspan="2"><input type="text" name="it_real_url" value="<?php echo $it['it_real_url']; ?>" id="it_real_url" class="frm_input" size="30"></td>
		</tr>


		<tr class="ac">
			<th scope="row"><label for="it_last_date">종료일시</label></th>
			<td colspan="2">
				<?php echo help("종료일시를 선택해주세요."); ?>
				<select name="it_last_date1" id="it_last_date1" class="frm_input required">
					<?for($i = date("Y"); $i <= date("Y") + 10; $i++){?>
					<option value="<?=$i?>" <?if($it_last_date1 == $i){echo "selected";}?>><?=$i?></option>
					<?}?>
				</select>년
				<select name="it_last_date2" id="it_last_date2" class="frm_input required">
					<?for($i = 1; $i <= 12; $i++){?>
					<option value="<?=$i?>" <?if($it_last_date2 == $i){echo "selected";}?>><?=$i?></option>
					<?}?>
				</select>월
				<select name="it_last_date3" id="it_last_date3" class="frm_input required">
					<?for($i = 1; $i <= 31; $i++){?>
					<option value="<?=$i?>" <?if($it_last_date3 == $i){echo "selected";}?>><?=$i?></option>
					<?}?>
				</select>일
				<select name="it_last_date4" id="it_last_date4" class="frm_input required">
					<?for($i = 1; $i <= 24; $i++){?>
					<option value="<?=$i?>" <?if($it_last_date4 == $i){echo "selected";}?>><?=$i?></option>
					<?}?>
				</select>시
				<select name="it_last_date5" id="it_last_date5" class="frm_input required">
					<?for($i = 0; $i < 60; $i++){?>
					<option value="<?=$i?>" <?if($it_last_date5 == $i){echo "selected";}?>><?=$i?></option>
					<?}?>
				</select>분
				<select name="it_last_date6" id="it_last_date6" class="frm_input required">
					<?for($i = 0; $i < 60; $i++){?>
					<option value="<?=$i?>" <?if($it_last_date6 == $i){echo "selected";}?>><?=$i?></option>
					<?}?>
				</select>초
			</td>
		</tr>
		...
		<tr>
			<th scope="row"><label for="it_complete_price">완판가격(목표달성)</label></th>
			<td>
				<?php echo help("입력하지 않으면 상품상세페이지에 출력하지 않습니다."); ?>
				 <input type="text" name="it_complete_price" value="<?php echo $it['it_complete_price']; ?>" id="it_complete_price" class="frm_input" size="8"> 원
			</td>
			<td class="td_grpset">
				<input type="checkbox" name="chk_ca_it_complete_price" value="1" id="chk_ca_it_complete_price">
				<label for="chk_ca_it_complete_price">분류적용</label>
				<input type="checkbox" name="chk_all_it_complete_price" value="1" id="chk_all_it_complete_price">
				<label for="chk_all_it_complete_price">전체적용</label>
			</td>
		</tr>		
				...
				<table border="0" cellspacing="0" cellpadding="0" width="100%" class="add_op">
					...
					<tr class='sub_tr'>
						<td>
							...
							<input type='text' class='frm_input' name='it_op_title[]' id='it_op_title' size='10' value='<?=$op_row[con]?>'>
							...
							<table border="0" cellspacing="0" cellpadding="0" width="100%" class="add_op_sub">
								...
								<tr>
									<td>
										...
										<input type="text" name="it_op_name<?=$i?>[]" value='<?=$op2_row[con]?>' id="it_op_name" class="frm_input" size="30">
										<label for="opt1"><b>금액</b></label>
										<input type="text" name="it_op_price<?=$i?>[]" value='<?=$op2_row[price]?>' id="it_op_price" class="frm_input" size="30">
										<input type="button" value="-" style="padding:0px 3px 14px 3px;height:18px;" onclick="add_op_del($(this));">
									</td>
								</tr>
								<?
			...
			<tr>
				...
				<td>
					...
					<select name="it_sc_type" id="it_sc_type">
						...
						<option value="3"<?php echo get_selected('3', $it['it_sc_type']); ?> selected>유료배송</option>
			...
			<tr id="sc_con_basic">
				...
				<td>
					<?php echo help("무료배송 이외의 설정에 적용되는 배송비 금액입니다."); ?>
					 <input type="text" name="it_sc_price" value="<?php if($it['it_sc_price']){echo $it['it_sc_price'];}else{echo "5000";} ?>" id="it_sc_price" class="frm_input" size="4"> 원
				</td>
			</tr>
...
$(document).ready(function(){
	...
	$(".add_op_bn").click(function(){
		...
		con += "<input type='text' class='frm_input' name='it_op_title[]' id='it_op_title' size='10' value='"+title+"'> ";
		...
		con += '<input type="text" name="it_op_name'+idx+'[]" value="" id="it_op_name" class="frm_input" size="30"> ';
		con += '<label for="opt1"><b>금액</b></label> ';
		con += '<input type="text" name="it_op_price'+idx+'[]" value="" id="it_op_price" class="frm_input" size="30"> ';
		con += '<input type="button" value="-" style="padding:0px 3px 14px 3px;height:18px;" onclick="add_op_del($(this));">';
		con += '</td>';
		con += '</tr>';
		con += '</table>';
...
<?php } ?>
	...
	$("#it_fix_price_type").live("click",function(){
		$(".realaddP").hide();
		$(".realUrl").hide();		
		$(".ac").hide();
	});
	...
	$("#it_auc_price_type").live("click",function(){
		$(".realaddP").hide();
		$(".realUrl").hide();
		$(".ac").show();
	});

	$("#it_url_price_type").live("click",function(){
		$(".realaddP").show();
		$(".realUrl").show();
		$(".ac").hide();
	});

	$("#it_metal_type").change(function(){
		if($(this).val()=="EC")$("#it_metal_etc_price_view").show();
		else $("#it_metal_etc_price_view").hide();
	});
});
