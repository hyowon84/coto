...
<div style="margin:0 0 10px 20px;">
	...
	<div class="order_modify" style="display:inline-block;padding:10px;border:1px solid #ccc;background:#f0f0f0;text-decoration:none;cursor:pointer">상태변경</div>
</div>
...
<div class="tbl_head02 tbl_wrap">
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td headers="th_delino" class="td_delino">
            ...
            <?php } else if ($od_status == '배송중' || $od_status ==  '배송완료') { ?>
...
<?php if (($od_status == '' || $od_status == '배송완료' || $od_status == '전체취소' || $od_status == '부분취소') == false) {
    // 검색된 주문상태가 '전체', '배송완료', '전체취소', '부분취소' 가 아니라면
?>
    <label for="od_status" class="cmd_tit">주문상태 변경</label>
    <?php
    $change_status = "";
    if ($od_status == '입금대기') $change_status = "결제완료";
    if ($od_status == '결제완료') $change_status = "상품준비중";
...
<?php } ?>
    <?php if ($od_status == '입금대기' || $od_status == '전체취소') { ?> <span>입금대기상태에서만 삭제가 가능합니다.</span> <input type="submit" value="선택삭제" class="btn_submit" onclick="document.pressed=this.value"><?php } ?>
</div>

<div class="local_desc02 local_desc">
<p>
    &lt;무통장&gt;인 경우에만 &lt;입금대기&gt;에서 &lt;결제완료&gt;으로 변경됩니다. 가상계좌는 입금시 자동으로 &lt;결제완료&gt;처리됩니다.<br>
...
$(function(){
	...
	$(".order_modify").click(function(){
		...
		if(confirm("선택하신 항목을 수정하시겠습니까?")){
			$("form[name='forderlist']").attr({"action":"./orderlist_wait_update.php"}).submit();
		}
	});
});
