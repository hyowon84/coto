...
<div class="local_ov01 local_ov">
    ...
    <?php if($od_status == '배송준비중' && $total_count > 0) { ?>
...
<div style="position:absolute;margin:-68px 0 0 0;text-align:right;right:0;padding:20px;">
	<div class="order_excel" style="display:inline-block;padding:10px;border:1px solid #ccc;background:#f0f0f0;text-decoration:none;cursor:pointer">엑셀다운로드</div>
</div>
...
<div class="tbl_head02 tbl_wrap">
    ...
    <tr class="orderlist<?php echo ' '.$bg; ?>">
		...
		<td rowspan="<?=$rowspan[cnt] * 3?>" class="td_odrnum2">
            <span style="font-size:14px;font-weight:bold;"><?=$item_deli_type?></span>
        </td>
		<?}?>
...
?>
    ...
    <?php
    $change_status = "";
    if ($od_status == '입금확인중') $change_status = "결재완료";
    if ($od_status == '결재완료') $change_status = "배송준비중";
	if ($od_status == '상품준비중') $change_status = "배송대기";
    if ($od_status == '배송준비중') $change_status = "배송중";
    ...
    <?php if($od_status == '입금확인중' || $od_status == '배송준비중') { ?>
...
<?php } ?>
    <?php if ($od_status == '입금확인중' || $od_status == '전체취소') { ?> <span>주문상태에서만 삭제가 가능합니다.</span> <input type="submit" value="선택삭제" class="btn_submit" onclick="document.pressed=this.value"><?php } ?>
</div>

<div class="local_desc02 local_desc">
<p>
    &lt;무통장&gt;인 경우에만 &lt;입금확인중&gt;에서 &lt;결재완료&gt;으로 변경됩니다. 가상계좌는 입금시 자동으로 &lt;결재완료&gt;처리됩니다.<br>
    &lt;배송준비중&gt;에서 &lt;배송중&gt;으로 변경시 &lt;에스크로배송등록&gt;을 체크하시면 에스크로 주문에 한해 KCP에 배송정보가 자동 등록됩니다.<br>
...
$(function(){
	...
	//엑셀 다운로드
	$(".order_excel").click(function(){
		$("form[name='frmorderlist']").attr("action", "./orderlist_deli_success_excel.php").submit();
	});
});
    ...
    */
    ...
    for (var i=0; i<chk.length; i++)
    {
        if (chk[i].checked)
        {
            ...
            {
                case "결제완료" :
                    if (!(current_status == "입금확인중" && current_settle_case == "무통장")) {
                        alert("'입금확인중' 상태의 '무통장'(결제수단)인 경우에만 '결제완료' 처리 가능합니다.");
                        return false;
                    }
                    break;
                ...
                case "배송준비중" :
                    if (current_status != "결제완료") {
                        alert("'결제완료' 상태의 주문만 '배송준비중'로 변경이 가능합니다.");
                        return false;
                    }
                    break;

                case "배송중" :
                    if (current_status != "배송준비중") {
                        alert("'배송준비중' 상태의 주문만 '배송중'으로 변경이 가능합니다.");
                        return false;
                    }
