...
<div class="tbl_head01 tbl_wrap">
    ...
    {
		$order_cnt = order_cnt_fn_gp($row[it_name]);			//주문건수
		$buy_cnt = buy_cnt_fn_gp($row[it_name]);				//판매건수
		$item_cnt = item_cnt_fn_gp($row[it_name]);				//판매물품갯수
		$buy_sum = buy_sum_fn_gp($row[it_name]);				//입금합계
		$buy_card_sum = buy_card_sum_fn_gp($row[it_name]);		//카드입금
		$buy_point_sum = buy_point_sum_fn_gp($row[it_name]);	//포인트입금
		$buy_mu_sum = buy_mu_sum_fn_gp($row[it_name]);			//무통장입금
		$buy_cancel_sum = buy_cancel_sum_fn_gp($row[it_name]);	//취소
		$buy_re_sum = buy_re_sum_fn_gp($row[it_name]);			//반품
		$buy_ex_sum = buy_ex_sum_fn_gp($row[it_name]);			//교환
		$buy_misu_sum = buy_misu_sum_fn_gp($row[it_name]);		//미수금
