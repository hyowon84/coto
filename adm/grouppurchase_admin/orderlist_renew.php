...
if ($search != "") {
	...
	$where[] = "( T.od_id in ( select od_id from {$g5[g5_shop_cart_table]} where 1=1	$검색조건 ) ";

	/* 닉네임 */
	$where[] = " M.mb_nick LIKE '%".$search."%' ";
	
	/* 주문자 */
	$where[] = " T.od_name LIKE '%".$search."%' ";
	
	/* 미입고 유(무) */
	//$where[] = " T.od_wearing_cnt > 0 ";
	
	/* 상품코드 */
// 	$where[] = " T.it_id LIKE '%$search%' ";
	
	/* 주문자연락처 */
	$where[] = " T.od_hp LIKE '%".$search."%' ";
	
	/* 주소 */
	$where[] = "( T.od_addr1 LIKE '%".$search."%' OR T.od_addr2 LIKE '%".$search."%' OR T.od_addr3 LIKE '%".$search."%' OR T.od_addr_jibeon LIKE '%".$search."%' )";
	
	/* 메모 */
	$where[] = " T.od_shop_memo LIKE '%".$search."%'
...
//$where[] = " T.gp_code <> '' and T.od_cart_price > 0";

if ($od_settle_case) {
	$where[] = " T.od_settle_case = '$od_settle_case' ";
}


if($sfl_code2 != ""){
	$where[] = " T.gp_code ='".$sfl_code2."' ";
}
...
if($od_status){
	$where2[] = " T.od_status = '$od_status' ";
}

if ($fr_date && $to_date) {
	$where2[] = " T.od_time between '$fr_date 00:00:00' and '$to_date 23:59:59' ";
}
...
if ($sel_field == "")  $sel_field = "T.od_id";
if (!$sst) {
	$sst = "T.od_id";
	$sod = "desc";
}
...
$sql = "	SELECT	COUNT(*) AS cnt,
					...
					FROM		(	SELECT	count(*) as cnt,
														sum(T.od_cart_price + T.od_send_cost + T.od_send_cost2) as total_price
										FROM		v_shop_order T
														LEFT JOIN g5_member M ON (M.mb_id = T.mb_id)
										$sql_search
										GROUP BY T.gp_no, T.mb_id
									) 	T
";

/*$sql = " select	count(*) as cnt,
						sum(od_cart_price + od_send_cost + od_send_cost2) as total_price " . $sql_common;
*/
...
/* 공동구매 회차별, 고객주문자별로 묶어서 보여줄수 있게 수정 by. JHW */
$sql_group = "GROUP BY T.gp_no, T.mb_id";
$sql_common = "FROM		v_shop_order T
											LEFT OUTER JOIN (	SELECT	CT.gp_no,
																								CT.mb_id,               
																								(CT.ct_price * (GG.gp_cart_qty - GG.gp_soldout) ) AS sum_gpTotalPrice,	/* 해당공구의 해당상품의 총 전체 주문금액을 말함 */
																								SUM(CT.ct_price * CT.ct_qty) AS sum_ct_price	/* 해당공구의 해당상품의 총 전체 주문금액을 말함 */
																				FROM		v_shop_cart CT
																								LEFT OUTER JOIN g5_shop_group_purchase_group GG ON (GG.gp_id = CT.it_id AND GG.gp_code = CT.total_amount_code)
																				WHERE		CT.ct_status NOT LIKE '%취소%'
																				GROUP	BY	CT.gp_no,CT.mb_id     
											) C ON (C.gp_no = T.gp_no AND C.mb_id = T.mb_id)
											LEFT OUTER JOIN g5_member M ON (M.mb_id = T.mb_id)
					$sql_search
					$sql_group	";
...
$sql  = " SELECT		T.gp_no,
										...
										T.od_remit,	/*예금주*/
										T.od_tax,	/*현금영수증여부*/
										T.tax_status,	/*영수증 발행 종류*/
										T.od_tax_hp,	/*영수증 발행 휴대폰*/
										T.od_last_date,	/*입금대기기간*/
										T.combine_deli_code,	/*통합배송 코드*/
										T.combine_deli_date,	/*통합배송 날짜*/
										T.combine_deli_status,	/*통합배송 여부*/
										T.od_wearing_cnt,	
										T.od_tax_state,	
										T.od_cart_count,	
										T.od_cart_price,	
										...
										T.od_tax_sort,	
										C.sum_ct_price,	/* 리얼합계 */
										C.sum_ct_price AS od_cart_price,
										SUM(T.od_cart_count) AS od_cart_count,
										
										SUM(T.od_send_cost) AS od_send_cost,
										SUM(T.od_send_cost2) AS od_send_cost2,
										                            
										SUM(T.od_cart_coupon) AS od_cart_coupon,
										SUM(T.od_cart_usd_price) AS od_cart_usd_price,
										SUM(T.od_exchange_rate) AS od_exchange_rate,
										                                 
										SUM(T.od_send_coupon) AS od_send_coupon,
										SUM(T.od_receipt_price) AS od_receipt_price,
										SUM(T.od_cancel_price) AS od_cancel_price,
										SUM(T.od_receipt_point) AS od_receipt_point,
										SUM(T.od_refund_price) AS od_refund_price,
										SUM(T.od_coupon) AS od_coupon,   
										SUM(T.od_misu) AS od_misu,       
										SUM(T.couponprice) AS couponprice
					$sql_common 
					$sql_order 
...
<div class="tbl_head01 tbl_wrap">
	...
	<tr>
		...
		<th scope="col" colspan='4' width='700' id="th_gpcode"><?=subject_sort_link('gp_code')?>상품정보</th>
		
		
		<!-- th scope="col" id="th_odrertel"><?=subject_sort_link('od_wearing_cnt')?>미입고유무</th-->
		<th scope="col">주문금액</th>
		<th scope="col">배송금액</th>
		<th scope="col"><?=subject_sort_link('od_cart_price')?>총금액</th>
		
		<th scope="col" id="th_nick"><?=subject_sort_link('mb_id')?>닉네임</th>
	...
	<tr class="orderlist<?=' '.$bg;?>">
		<td class="td_chk">
			...
			<input type="checkbox" class='selectAllDetail' value="chk_<?=$i?>" id="chk_<?=$i?>" />
		</td>
		<td headers="th_ordnum" class="td_odrnum2" style='text-align:left;' colspan='4'>
			
			<table width='100%' cellpadding='0' cellspacing='0'>
			<!-- tr>
				
				<th scope="col" id="th_odrertel">수량</th>
				<th scope="col" id="th_odrertel">진행상태</th>
				<th scope="col" id="th_odrertel">출력</th>
			</tr-->
			<tr>
				<th scope="col" class='ordertable_td' align='center'><b>상품명</b></th>
				<th scope="col" class='ordertable_td' align='center'><b>수량</b></th>
				<th scope="col" class='ordertable_td' align='center'>단가</th>
				<th scope="col" class='ordertable_td' align='center'><b>진행상태</b></th>
				<th scope="col" id="th_odrertel">출력</th>
			</tr>
			<!-- tr>
			...
			<?	
				/* 주문내역상세보기 */
				$odinfo_sql = "	SELECT	DISTINCT 
															OD.od_id
											FROM		g5_shop_order OD
											WHERE		1=1
											AND		REPLACE(SUBSTRING(SUBSTRING_INDEX(OD.gp_code, '_', 2),LENGTH(SUBSTRING_INDEX(OD.gp_code,  '_',1-1)) + 1), '_', '') LIKE '%$row[gp_no]%'
											AND		OD.mb_id = '$row[mb_id]'
											ORDER BY OD.od_id DESC
				";
				$odinfore = sql_query($odinfo_sql);
				$주문서개수 = mysql_num_rows($odinfore);
				
				while($odinfo = mysql_fetch_array($odinfore)) {
				?>
					<a href="./orderform.php?od_id=<?=$odinfo['od_id'];?>&amp;<?=$qstr;?>" class="mng_mod">
						<span class="sound_only"><?=$odinfo['od_id'];?> </span>[주문서상세보기 ▶<?=$odinfo['od_id']?>]
					</a>
				<?
				}
				?>
			...
			$cart_sql = "	SELECT	C.*,
													...
													GC.ca_name
									FROM		g5_shop_cart C
													LEFT OUTER JOIN g5_shop_order O ON (O.od_id = C.od_id)
													LEFT OUTER JOIN g5_shop_category GC ON (GC.ca_id = C.ct_type)
									WHERE		1=1
									AND		REPLACE(SUBSTRING(SUBSTRING_INDEX(C.total_amount_code, '_', 2),LENGTH(SUBSTRING_INDEX(C.total_amount_code,  '_',1-1)) + 1), '_', '') LIKE '%$row[gp_no]%'
									AND		C.mb_id = '$row[mb_id]'
									ORDER BY	C.total_amount_code ASC
			";
			$cart_result = sql_query($cart_sql);
			
			if($mode == 'jhw') echo "<textarea style='margin-bottom:10px;'>$cart_sql</textarea>";
			
			
			while($cart = mysql_fetch_array($cart_result)) {
				$현재공구코드 = $cart[total_amount_code];
				
				$진행상태 = $cart[ct_status];
				
				$ct_status_color = "";
				switch($cart[ct_status]){
					case "입금대기":
						$ct_status_color = "red";
						break;
					case "결제완료":
						$ct_status_color = "green";
						break;
					case "상품준비중":
						$ct_status_color = "purple";
						break;
					case "배송중":
					case "배송완료":
						$ct_status_color = "blue";
						break;
				}
				$ct_status_color = ($ct_status_color) ? "style='color:$ct_status_color'" : ''; 
				
				if($현재공구코드 != $이전공구코드) {
			?>
					<tr><td class='ordertable_td title' colspan='5'>
						▼<?="[".$현재공구코드."]"?> |
						<input type='button' onclick="print_order('<?=$cart[ct_id]?>','<?=$cart[print_yn]?>','GP','Y')" value='인쇄(모두)' />
						<input type='button' onclick="print_order('<?=$cart[ct_id]?>','<?=$cart[print_yn]?>','GP','N')" value='인쇄(기존출력건 제외)' />
						<input type='button' onclick="print_order('<?=$cart[ct_id]?>','N','CHOICE','Y')" value='선택인쇄(체크된것만)' />
					</td></tr>
			<?
				}
			?>
					<tr>
						<!--td class='ordertable_td'><?=$cart[od_id]?></td-->
						<td class='ordertable_td' title='<?=$cart[it_name]?>' style='text-align:left;'>
							<input type="checkbox" class="chkCart chk_<?=$i?>" name="ct_id[<?=$cart_cnt?>]" value="<?=$cart['ct_id']?>" id="ct_id_<?=$cart_cnt?>">
							<?="<a href='./orderform.php?od_id=$cart[od_id]&amp;$qstr;' class='mng_mod'><font color=blue>[$cart[od_id]]</a></font><font color=red>[$cart[ct_id]]</font> [".$cart[ca_name]."]".substr($cart[it_name],0,80).'..'?>
						</td>
						<td class='ordertable_td'><?=$cart[ct_qty]?></td>						
						<td class='ordertable_td' style='text-align:right;'><?=number_format($cart[ct_price])?></td>
						<td class='ordertable_td' <?=$ct_status_color?>><?=$진행상태?></td>
						<td class='ordertable_td'>
							<input type='button'  onclick="print_order('<?=$cart[ct_id]?>','<?=$cart[print_yn]?>','CART','N')" value='<?=$cart[print_yn]?>' />
						</td>
					</tr>
			<?
					$이전공구코드 = $cart[total_amount_code];
					$cart_cnt++;
					//if($cart_cnt > 0) $줄바꿈 = "<br><br>";
			}
			$줄바꿈 = '';
			$현재공구코드 = '';
			$이전공구코드 = '';
...
<style>
	.tbl_head01 thead th, .tbl_head01 tbody th { padding:0px; }
	.ordertable_td { background-color:white; }
</style>

<script>
/* 제일좌측 체크박스 클릭시 바로 우측 체크박스 전체선택 및 전체해제 이벤트 */
$('.selectAllDetail').on('click',function(e){
...
function print_order(ct_id,yn,type,all_yn) {
	...
	if(quest) {
		...
		if(type == 'CHOICE') {
			...
			ct_id = v_list.substring(0,v_list.length-1);
		}
		window.open('order_printpage.php?ct_id='+ct_id+'&type='+type+'&all_yn='+all_yn);
	}
	return;
}
...
function forderlist_submit(f)
{
	...
	f.action = "./orderlistupdate_renew.php";
	return true;
}
