...
<div style="margin:0 0 10px 20px;">
	<select name="od_status" style="height:36px;">
		<option value="상품준비중">해외배송대기</option>
		<option value="취소">주문취소</option>
	</select>
	<div class="order_modify" style="display:inline-block;padding:10px;border:1px solid #ccc;background:#f0f0f0;text-decoration:none;cursor:pointer">선택수정</div>
</div>
...
$(function(){
	...
	$(".order_modify").click(function(){
		...
		if(confirm("선택하신 항목을 수정하시겠습니까?")){
			$("form[name='forderlist']").attr({"action":"./orderlistupdate.php"}).submit();
		}
	});
});
