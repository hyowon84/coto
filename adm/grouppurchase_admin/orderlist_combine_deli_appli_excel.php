...
header("Content-Type: application/x-msexcel; name=\"gp-".date("ymd", time()).".xls\"");
header("Content-Disposition: inline; filename=\"gp-".date("ymd", time()).".xls\"");
...
if ($od_status) {
    switch($od_status) {
        case '전체취소':
            ...
            break;
        case '부분취소':
            $where[] = " od_status IN('입금대기', '결제완료', '해외배송대기', '해외배송중', '상품준비중', '배송대기', '배송중', '배송완료') and od_cancel_price > 0 ";
            break;
        default:
