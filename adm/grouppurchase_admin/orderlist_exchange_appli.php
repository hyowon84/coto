...
$where[] = " od_id in ( select distinct od_id from $g5[g5_shop_cart_table] where ct_gubun='P' and ex_status='y' ) ";
$where[] = " od_status!='교환대기' and od_status!='반품' and od_status!='교환' ";
...
<div style="margin:0 0 10px 20px;">
	<select name="od_status" style="height:36px;">
		<option value="교환대기">교환대기</option>
	</select>
	<div class="order_modify" idx="1" style="display:inline-block;padding:10px;border:1px solid #ccc;background:#f0f0f0;text-decoration:none;cursor:pointer">선택수정</div>
</div>
...
$(function(){
	...
	$(".order_modify").click(function(){
		...
		if(confirm("선택하신 항목을 수정하시겠습니까?")){
			...
			$("form[name='forderlist']").attr({"action":"./orderlist_exchange_appli_update.php"}).submit();
		}
	});
});
