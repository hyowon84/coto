...
$g5['title'] = '모바일 메인 슬라이드 관리';
include_once('./admin.head.php');

if($no){
	$mode = "u";
	$mobile_main_slide_row = sql_fetch("select * from g5_mobile_main_slide where no='$no' ");
}else{
	$mode = "w";
}
...
<form name="fmain_silde" action="mobile_main_slide_update.php" method="POST" enctype="multipart/form-data">
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <td>
            <input type="file" name="img_file"> <?=$mobile_main_slide_row[img_file]?>
        </td>
    </tr>
	<tr>
		<th scope="row">URL</th>
		<td>
			<input type="text" style="width:300px;height:20px;" name="URL" value="<?=$mobile_main_slide_row[URL]?>">
		</td>
	</tr>
	<tr>
		...
		<td>
			<?if($no){?>
				<input type="checkbox" name="status" value="2" <?if($mobile_main_slide_row[status] == "2"){ echo "checked";}?>>
			<?}else{?>
...
<?if($mobile_main_slide_row[back_status] != "1" && $mobile_main_slide_row[back_status] != "2"){?>
	$(".back_dis").hide();
<?}?>
	...
	$(".list").click(function(){
		goto_url('./mobile_main_slide.php');
	});
});
