...
if ($stx != "") {
    if ($sfl != "") {
		if($sfl == "mb_nick"){
			$mem = sql_fetch("select * from {$g5['member_table']} where mb_nick='".$stx."'");
			$sql_search .= " $where a.mb_id like '".$mem[mb_id]."' ";
		}else{
...
<div class="tbl_head02 tbl_wrap">
    ...
    for ($i=0; $row=mysql_fetch_array($result); $i++)
    {
		...
		$gp = sql_fetch("select * from {$g5['g5_shop_group_purchase_table']} where gp_id='".$row[it_id]."' limit 0, 1 ");

		$cart_cnt = sql_fetch("select count(*) as cnt from {$g5['g5_shop_cart_table']} where mb_id='".$row[mb_id]."' and ct_status in('쇼핑', '입금대기') and ct_gubun='P' ");
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td>
			닉네임 : <?php echo $mem[mb_nick]; ?></br>
			전화번호 : <?php echo $mem[mb_hp]; ?></br>
			메일 : <?php echo $mem[mb_email]; ?>
        </td>
        <!--<td class="td_img">
            <?=$image?>
        </td>-->
		<td>
			...
			<span style="color:blue;cursor:pointer;">
				<a href="./grouppurchase_appli_sum1_view.php?mb_id=<?=$row[mb_id]?>" style="color:red;">상세보기</a>
			</span>
		</td>
		<td class="td_num">
