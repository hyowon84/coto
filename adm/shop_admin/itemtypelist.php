...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
		...
		<th scope="col">선택없음</th>
		<?
		$icon_tit_res = sql_query("
			select * from {$g5['g5_item_type_icon_table']}
			order by no asc
		");

		for($i = 0; $icon_tit_row = mysql_fetch_array($icon_tit_res); $i++){
		?>
		<th scope="col"><?=$icon_tit_row[tp_name]?></th>
		<?
		}
		?>
    ...
    <tr class="<?php echo $bg; ?>">
		...
		<td class="td_chk"><input type="radio" name="it_type<?php echo $i; ?>" value="0" <?php echo ($row['it_type'] == "0" ? 'checked' : ''); ?>></td>
		<?
		$icon_res = sql_query("
			select * from {$g5['g5_item_type_icon_table']}
			order by no asc
		...
		?>
        <td class="td_chk"><input type="radio" name="it_type<?php echo $i; ?>" value="<?=$icon_row[no]?>" <?php echo ($row['it_type'] == $icon_row[no] ? 'checked' : ''); ?>></td>
		<?
		}
		?>
