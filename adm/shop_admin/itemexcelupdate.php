...
if($_FILES['excelfile']['tmp_name']) {
    ...
    */
    ...
    for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
        ...
        $it_year            = addslashes($data->sheets[0]['cells'][$i][$j++]);
        $it_name            = addslashes($data->sheets[0]['cells'][$i][$j++]);
        $it_mobile_name     = addslashes($data->sheets[0]['cells'][$i][$j++]);
		$it_metal_type     = addslashes($data->sheets[0]['cells'][$i][$j++]);
		$it_metal_don     = addslashes($data->sheets[0]['cells'][$i][$j++]);
        ...
        $it_7           = addslashes($data->sheets[0]['cells'][$i][$j++]);
        $it_type           = addslashes($data->sheets[0]['cells'][$i][$j++]);
        ...
        $it_price_type   = addslashes($data->sheets[0]['cells'][$i][$j++]);
		$it_real_add_price           = addslashes(only_number($data->sheets[0]['cells'][$i][$j++]));
        $it_real_add_unit   = addslashes($data->sheets[0]['cells'][$i][$j++]);
        $it_real_url   = addslashes($data->sheets[0]['cells'][$i][$j++]);
		$it_last_date           = addslashes(only_number($data->sheets[0]['cells'][$i][$j++]));
		...
		$it_type1= $it_type2 = $it_type3 = $it_type4 = $it_type5 = $it_type6 ="";
        ...
        $row2 = sql_fetch($sql2);
        if($row2['cnt']) {
			$sql = "update {$g5['g5_shop_item_table']} $sql_common where it_id = '$it_id' ";
			sql_query($sql);
        }else{
			$sql = " INSERT INTO {$g5['g5_shop_item_table']} $sql_common , it_id = '$it_id'";
			sql_query($sql);
		}
