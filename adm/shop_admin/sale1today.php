...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
		...
		$gubun = sql_fetch("select * from {$g5['g5_shop_cart_table']} where od_id='".$row[od_id]."' limit 0, 1 ");

		if($gubun[ct_gubun] == "P"){
			$shop_course = "../grouppurchase_admin";
		}else{
			$shop_course = "../shop_admin";
		}
    ?>
        <tr>
            <td class="td_alignc"><a href="<?=$shop_course?>/orderform.php?od_id=<?php echo $row['od_id']; ?>"><?php echo $row['od_id']; ?></a></td>
