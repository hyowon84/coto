		...
		switch($sel_field){
			case "it_name":
				...
				for($s = 0; $s < count($분할_검색어); $s++) {
					$검색조건 .= " AND	a.it_name LIKE '%".$분할_검색어[$s]."%' ";
				}
...
$sql = " select count(*) as cnt
		   from {$g5['g5_shop_cart_table']} a 
		   		left join {$g5['g5_shop_group_purchase_table']} b on ( a.it_id = b.gp_id )
...
echo "<textarea style='height:150px;'>$sql</textarea>";
...
<div class="sch_last">
	...
	<select name="sel_field" id="sel_field">
		...
		<option value="ct_wearing_cnt" <?=get_selected($sel_field, 'ct_wearing_cnt')?>>미입고(유)</option>
		<option value="total_amount_code" <?=get_selected($sel_field, 'total_amount_code')?>>공동구매코드</option>
	</select>
...
<div class="btn_add01 btn_add">
	...
	<div style="clear:both;">
		<div class="btn_list01 btn_list" style="float:left;">
			...
			<input type="button" id="btnDenySell" value="판매취소">
			<?php } ?>
			
			<input type="button" id="btnUpdateProduct" value="상품정보업데이트">
			
			<a href="#" class="go_excel">엑셀다운로드</a>
			<?php if ($is_admin == 'super') { ?>
			
			
			<div style='margin-left:10px; float:right; border:1px solid red;'>
				
				<input type='text' id='txt_price' name='txt_price' value='' />
				<input type="button" id="btnModPrice" value="가격수동입력">
			</div>
			<?php } ?>
		</div>
	</div>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="<?=$bg?>">
		...
		<td class="td_num"><?=printGrouppurCode($row['total_amount_code'])?></td>
        <td class="td_img">
            <?=$image?>
        </td>
		<td>
			<div>
				<a href="<?=$href?>"><?=$row[it_name]?></a>
			</div>
		...
		<td class="td_num"><?=$row[ct_qty]?>개</td>
		...
		<td class="td_numbig"><?=number_format($totalPrice)?> 원</td>
        <td class="td_mngsmall go_status">
        	<?=getPurchaseStateText($row['total_amount_code']);?><BR>
        	<?=$row[ct_status]?>        
        </td>
...
//	});
	...
	$("#btnModPrice").click(function(){
		...
		v_price = number_format($('#txt_price').val());
		
		if(confirm("선택한 자료를 "+v_price+"원에 변경하시겠습니까?")) {
			$("input[name='mode']").val("modify_price");
			$("form[name='fitemlistupdate']").submit();
		}		
	});

	/* MULTI 판매취소 버튼 */
	$("#btnDenySell").click(function(){
		...
		if($(this).val() == "판매취소") {
			if(!confirm("선택한 자료를 판매취소하시겠습니까?")) {
				return false;
			}
			$("input[name='mode']").val("denysell");
		}
