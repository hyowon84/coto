...
// 상품문의의 개수를 얻음
$sql = " select count(*) as cnt from `g5_shop_gpitem_qa` where gp_id = '{$gp_id}' ";
$row = sql_fetch($sql);
...
<div id="sit">
	<?php if (substr($ca_id, 0, 2) == "10") {?>
		<ul id="m_store_cate">
			<li><a href="<?php echo G5_SHOP_URL ?>/list.php?ca_id=1010" <?php if(substr($_GET[ca_id], 2, 2) == "10") echo " class='m_store_cate_on'";?>>GOLD</a></li>
			<li><a href="<?php echo G5_SHOP_URL ?>/list.php?ca_id=1020" <?php if(substr($_GET[ca_id], 2, 2) == "20") echo " class='m_store_cate_on'";?>>SILVER</a></li>
			<li><a href="<?php echo G5_SHOP_URL ?>/list.php?ca_id=1030" <?php if(substr($_GET[ca_id], 2, 2) == "30") echo " class='m_store_cate_on'";?>>OTHER METAL</a></li>
		</ul>
		<?php } ?>
	<?php
	//홈 > 일반상품 > ...  네비게이션
	...
	$경매여부 = ($it['ca_id'] == '1010' || $it['ca_id2'] == '1010' || $it['ca_id3'] == '1010') ? true : false;

	if($경매여부) {
		$상품유형 = '경매상품';
		$상품구입폼 = G5_MSHOP_SKIN_PATH.'/item.form.auction.skin.php';
		$상품상세폼 = G5_MSHOP_SKIN_PATH.'/item.info.auction.skin.php';
	} else {
		$상품유형 = '일반상품';
		$상품구입폼 = G5_MSHOP_SKIN_PATH.'/item.form.skin.php';
		$상품상세폼 = G5_MSHOP_SKIN_PATH.'/item.info.skin.php';
	}

	?>
	<link rel="stylesheet" href="<?php echo G5_MSHOP_SKIN_URL; ?>/style.css">

	<section id="itemdtl_top">
		<div class='itemdtl_top_title'><?=$상품유형?></div>
		<div class="itemdtl_top_year ellipsis"><?php echo $it['it_year']; ?></div>
		<div class="itemdtl_top_tit line1row ellipsis"><?php echo stripslashes($it['it_name']); ?></div>

		<?
		if($카테고리 == '1010') {
		?>
		<div class='itemdtl_top_maker'>제조사 : <?php echo $it['it_maker']; ?></div>
		<?
		}
		?>
	</section>
