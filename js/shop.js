...
$(function() {
    ...
    if(isAndroid) {
		...
		$("select.it_option").live("mousedown, click", function() { 
            option_add = true;
        });
    }
	...
	// 수량변경 및 삭제
    $("#sit_sel_option1 li button").live("click", function() {
        var mode = $(this).text();
        var this_qty, max_qty = 9999, min_qty = 1;
        var $el_qty = $(this).closest("li").find("input[name^=ct_qty]");
        var stock = parseInt($(this).closest("li").find("input.io_stock").val());
...
function price_calculate()
{
	...
	var $el_ct_bid = $("input[name=ct_bid]");
    ...
    $el_prc.each(function(index) {
        ...
        if(type == "0") { // 선택옵션
            total += (it_price + price) * qty + parseInt(op_price);
        } else { // 추가옵션
            total += price * qty + parseInt(op_price);
        }
    });

	if($el_ct_bid.val() != "" && $el_ct_bid.val() != "undefined"){
		total = total + parseInt($el_ct_bid.val());
	}

    //$("#sit_tot_price").empty().html("총 금액 : "+number_format(String(total))+"원");
	$("#sit_tot_price").empty().html(number_format(String(total))+"<span style='font-size:16px;'>원</span>");
}
