    ...
    function html5Upload() {	
		...
		sUploadURL= 'FileUploader_html5.php?bo_table='+bo_table; 	//upload URL
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
			...
			sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/FileUploader.php?bo_table='+bo_table,	//샘플 URL입니다.
 	...
 	function setPhotoToEditor(oFileInfo){
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			for(var i in oFileInfo){ 
				opener.parent.setEditorFileList(oFileInfo[i].sFileOrgName, oFileInfo[i].sFileName, oFileInfo[i].sFileURL, oFileInfo[i].sFileSize, oFileInfo[i].bNewLine);
			} 
