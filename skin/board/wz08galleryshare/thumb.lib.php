...
$width에 값이 없으면 (높이로만 정렬하는 갤러리의 경우)
    ...
    - else
        ...
        - else : 비율대로 썸을 생성 
*/
function thumbnail($file_name, $width=0, $height=0, $is_create=false, $is_crop=2, $quality=90, $small_thumb=1, $thumb_dir="", $watermark="", $filter="", $noimg="", $thumb_type="") 
{
    //if (!$file_name)
    //    return;
    ...
    if (is_array($watermark)) {
		...
		switch ($watermark[location]) {
		  case "center"       : 
				$tx = (int)($sx/2 - $wx/2) + $watermark[x];
				$ty = (int)($sy/2 - $wy/2) + $watermark[y];
				break;
		  case "top"          :
				$tx = (int)($sx/2 - $wx/2) + $watermark[x];
				$ty = $watermark[y];
		  ...
		  case "top_left"     :
				...
				break;
		  case "top_right"    :
				$tx = $sx - $wx - $watermark[x];
				$ty = $watermark[y];
				break;
		  case "bottom"       :
				$tx = (int)($sx/2 - $wx/2) + $watermark[x];
				$ty = $sy - $watermark[y] - $wy;
				break;
		  case "bottom_left"  :
				$tx = $watermark[x];
				$ty = $sy - $watermark[y] - $wy;
