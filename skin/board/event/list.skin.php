...
<div id="bo_gall" style="width:<?php echo $width; ?>">
    ...
    <ul id="gall_ul">
        <?php for ($i=0; $i<count($list); $i++) {
         ...
         ?>
        <li class="gall_li <?php if ($wr_id == $list[$i]['wr_id']) { ?>gall_now<?php } ?>" style="<?php echo $style ?>width:<?php echo $board['bo_gallery_width'] ?>px;<?=$margin?>">
            <?php if ($is_checkbox) { ?>
            ...
            <ul class="gall_con">
				...
				<?if(strtotime("now") > $list[$i]['wr_5']){?>
                    마감
				<?}else if(strtotime("now") < $list[$i]['wr_4']){?>
					대기
				<?}else{?>
					진행중
				<?}?>
				...
				<li class="gall_text_comments">
                    ...
                    //if (isset($list[$i]['icon_secret'])) echo $list[$i]['icon_secret'];
                     ?>
					<?//=cut_str($list[$i]['wr_content'], 70)?>
				</li>
                <li class="gall_text_date">
					이벤트기간</br>
					<?php echo date("Y-m-d", $list[$i]['wr_4'])?> ~
					<?if($list[$i]['wr_6'] != "y"){?>
						<?php echo date("Y-m-d", $list[$i]['wr_5'])?>
					<?}?>
				</li>
