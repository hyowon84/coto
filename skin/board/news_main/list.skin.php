        ...
        <li class="gall_li <?php if ($wr_id == $list[$i]['wr_id']) { ?>gall_now<?php } ?>" style="<?php echo $style ?>width:<?php echo $board['bo_gallery_width'] ?>px">
            ...
            <ul class="gall_con">
                <li class="gall_href">
                    ...
                    <?php } else {
                        $thumb = get_list_thumbnail1($board['bo_table'], $list[$i]['wr_id'], $board['bo_gallery_width'], $board['bo_gallery_height']);
				...
				<li class="gall_text_comments gall_text_comments<?=$i?>" con='<?=$list[$i]['wr_content']?>' getConNum="<?=$i?>"><?//=cut_str($list[$i]['wr_content'], 70)?></li>
                <li class="gall_text_date"><?php echo $list[$i]['datetime'] ?></li>
...
$(document).ready(function(){
	...
	var con;
	var getCon;
	$(".gall_text_comments").each(function(i){
		con = "";
		getCon = "";
		con = $(".gall_text_comments").eq(i).attr("con");
		$(".getContent").html(con);
		$(".getContent").find("img").remove();
		if($(".getContent").text().length > 70){
			getCon = $(".getContent").text().substring(0, 100) + "...";
		}else{
			getCon = $(".getContent").text();
		}
		$(".gall_text_comments").eq(i).html(getCon);
	});
});
