...
<div style="background:#fff;padding:30px 10px 0 10px;">
	<!-- 전체 상품 사용후기 목록 시작 { -->
	...
	<div id="sps_sch">
		...
		<select name="sfl" required id="sfl">
			...
			<option value="b.gp_name"   <?php echo get_selected($sfl, "b.gp_name"); ?>>상품명</option>
	...
	<div id="sps">
		...
		for ($i=0; $row=sql_fetch_array($result); $i++)
		{
			...
			$small_image = $row['gp_id'];

			$row2 = sql_fetch(" select gp_name from {$g5['g5_shop_group_purchase_table']} where gp_id = '{$row['gp_id']}' ");
		...
		<li>
			...
			<div class="sps_img">
				<a href="<?php echo $it_href; ?>">
					...
					<span><?php echo $row2['gp_name']; ?></span>
				</a>
			</div>

			<section class="sps_section">
				<h2><?php echo $row['gp_name']; ?></h2>
		...
		<?php }
		if ($i > 0) echo '</ol>';
		if ($i == 0) echo '<p id="sps_empty" style="text-align:center;">자료가 없습니다.</p>';
		?>
	</div>
</div>
