...
.sanchor li{width:20%;}
.sanchor li a{width:100%;text-align:center;padding:0;background:#fff;}
...
<?php if($it['ca_id']!="1040"){?>
<div style="background:#fff;height:280px;padding:10px;">
	...
	<!-- 이 상품을 본 고객이 구매한 다른 상품 시작 { -->
	<section id="sit_rel">
		<!--<h2>이 상품을 본 고객이 구매한 다른 상품</h2>-->
		<div class="sct_wrap">
			<?php
	...
	<div style="height:390px;margin:0 -10px 0 -10px;">
		...
		<div id="sliderContainer" style="height:350px;">
			<ul id="simpleSlider1" style="height:350px;">
				...
				?>
					<li style="float:left;position:absolute;cursor:pointer;width:100%;height:350px;"><img src="../data/store_view_slide/<?=$store_row[img_file]?>" border="0" align="absmiddle" style="width:798px;height:350px;"></li>
				<?
				}
				?>
		...
		<div style="margin:10px 0 0 20px;color:#545454;font-size:12px;">※ 베스트 갤러리에 뽑히신 회원님에게는 소정의 사은품을 증정합니다.</div>

	</div>
	<!-- 포토갤러리 슬라이드 끝 -->
	<div style="margin:10px 0 0 0;">
		<div style="float:right;width:130px;height:15px;padding:15px;text-align:center;border:1px #cfcfcf solid;cursor:pointer;" onclick="goto_url('<?=G5_URL?>/bbs/board.php?bo_table=photo_gallery');">포토갤러리 가기 ></div>
	</div>
</div>
<div style="height:10px;"></div>
<?php }?>
...
<div id="sit_inf">
    ...
    <div id="sit_inf_basic">
		...
		<div style="margin:20px 0 20px 0;font-weight:bold;"><?php echo $it['it_basic']; ?></div>
		...
		<table class="sit_ov_tbl">
			...
			for($j=1;$j<=10;$j++){
				if ($it['it_'.$j]) { ?>
			<tr>
				<th scope="row"><?php echo $it['it_'.$j.'_subj']; ?></th>
				<td><?php echo $it['it_'.$j]; ?></td>
			</tr>
			<?php }
			}?>
		</table>
...
<section id="sit_dvr">
	...
	<div style="padding:30px 20px 10px 20px; margin:0px auto; width:800px;">
