...
while($row = mysql_fetch_array($result)) {
	...
	if(!$Response || eregi("<h1>An error occurred.</h1>",$Response) ) {
		$fail_count++;
	...
	if(stristr($gp_site,"apmex.com")){
		//APMEX - 상품명 추출
		preg_match_all($정규패턴['APMEX']['상품명'],$Response,$match_title);
		...
		if( !(strlen($gp_name) > 5) ) {
			echo "상품명 : $gp_name <br>상품명 존재안함 종료.";
			print_r($Response);
			exit;
		}
		...
		for($i=1; $i < count($cate_tmp)-1; $i++) {
			...
			if(!$srow[ca_id]) {
				...
				sql_query($ins_sql);
				echo $ins_sql."<br>";
				db_log($ins_sql,'g5_shop_category',"딜러업체상품 업데이트 > 카테고리");
			}
			else {
				$연결카테고리 = $srow[ca_id];
