...
<div class="ins_inner">
	...
	<div class="inner_btn">
		<a href="<?php echo G5_URL; ?>/install/"><?=G5_VERSION?> 설치하기</a>
	</div>
</div>
<div id="ins_ft">
...
ini_set("session.cache_expire", 900); // 세션 캐쉬 보관시간 (분)
ini_set("session.gc_maxlifetime", 90800); // session data의 garbage collection 존재 기간을 지정 (초)
...
//아이디가 없는 사용자를 위한 개인세션번호 생성 - 장바구니 기능
$랜덤세션 = rand(0000,1000).'-'.rand(0000,1000).'-'.rand(0000,1000);

if( !(strlen($_SESSION[ss_id]) > 3) ) {
	$_SESSION[ss_id] = $랜덤세션;
}
...
if (G5_USE_MOBILE) {
	...
	if ($_REQUEST['device']=='mobile' || $모바일검증) {
		$is_mobile = true;
	}
	else if ($_REQUEST['device']=='pc' || !$모바일검증) {
		$is_mobile = false;
	}
...
if ($member['mb_id']) {
	...
	/* 관리자여부 값이 Y이면 true, 아닐경우 기존 체크 방식대로 체 */
	if($member[admin_yn] == 'Y') {
		$is_admin = 'super';	/* 해당 관리자는 최고관리자로 설정 */
	} else {
