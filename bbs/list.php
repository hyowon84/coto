...
if ($sca || $stx) {
    $sql = " select distinct wr_parent from {$write_table} where {$sql_search} {$sql_que} {$sql_order} limit {$from_record}, $page_rows ";
} else {
    $sql = " select * from {$write_table} where wr_is_comment = 0 {$sql_que} {$sql_order} limit {$from_record}, $page_rows ";
}
$result = sql_query($sql);
...
//$write_pages = get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, './board.php?bo_table='.$bo_table.'&ca_name='.$ca_name.$qstr.'&amp;page=');

$list_href = '';
$prev_part_href = '';
$next_part_href = '';
if ($sca || $stx) {
    $list_href = './board.php?bo_table='.$bo_table.'&ca_name='.$ca_name;
...
<script>
    $(function() {
        $('#page-selection').bootpag({
            ...
            location.href = "<?='./board.php?bo_table='.$bo_table.'&ca_name='.$ca_name.$qstr.'&page='?>" + num;
        });
    });
</script>
