...
if($w == ''){
	if($_FILES[mb_img][tmp_name]){
		$filename_arr = explode(".", $_FILES[mb_img][name]);
		$filename = $filename_arr[0]."_profile_".strtotime("now");
		$ext = $filename_arr[1];
		$imgname = $filename.".".$ext;
		move_uploaded_file($_FILES[mb_img][tmp_name], $path.$imgname);
	}
}else if($w == "u"){
	if($_FILES[mb_img][tmp_name]){
		$img_chk = sql_fetch("select * from {$g5['member_table']} where mb_id = '$mb_id'");
		unlink($path.$img_chk[mb_img]);
